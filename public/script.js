...
(() => {
  ...
  function symbolLabel(sym){
    ...
    if(state.symbolType === 'emoji') return sym;
    const last = String(sym).split('/').pop() || String(sym);
    return last.replace(/\.[a-z0-9]+$/i, '');
  }
  function readDomMiddleSymbols(){
    const out = [];
    if(!reelsEl) return out;
    for(let i=0;i<state.columns;i++){
      const reel = reelsEl.children[i];
      const inner = reel?.querySelector('.reel-inner');
      // Middle visible row when transform=0 is children[1]
      const el = inner?.children?.[1];
      out.push(el?.dataset?.key ?? null);
    }
    return out;
  }
  function logSpinPlan(beforeArr, afterArr){
    const rows = beforeArr.map((b, i) => ({
      Reel: i+1,
      Before: symbolLabel(b),
      After: symbolLabel(afterArr[i]),
      Held: state.holds.has(i),
      Changed: b !== afterArr[i]
    }));
    try{ console.groupCollapsed('[DEBUG] Spin plan'); console.table(rows); console.groupEnd(); }catch{}
    try{ devLog('Plan: ' + rows.map(r=>r.Reel).join(' - ')); }catch{}
  }
  function logSpinResult(expectedArr){
    const actualArr = readDomMiddleSymbols();
    const rows = expectedArr.map((exp, i) => ({
      Reel: i+1,
      Expected: symbolLabel(exp),
      Actual: symbolLabel(actualArr[i]),
      Match: exp === actualArr[i]
    }));
    try{ console.groupCollapsed('[DEBUG] Spin result'); console.table(rows); console.log('Win:', state.win, 'Bet:', state.bet); console.groupEnd(); }catch{}
    const allMatch = rows.every(r => r.Match);
    try{ devLog('Resultat: ' + rows.map(r=>r.Reel).join(' - ')); }catch{}
    try{ devLog(allMatch ? 'Resultat OK' : 'Resultat DIFFERENT'); }catch{}
  }
  ...
  const Sound = {
    ...
    themeKeys: new Set(), // keys loaded from the current theme
    _unlocked: false,
    ambientNode: null,
    ...
    // Load global default audio (used if theme doesn't provide one)
    async loadDefaultAudio(base='assets/audio'){
      this.init();
      const ensureSlash = (p) => p.endsWith('/') ? p : (p + '/');
      const root = ensureSlash(base);
      const candidates = {
        uiClick: ['button-click.wav', 'button-click.mp3'],
        spin: ['spin-start.wav', 'spin-start.mp3'],
        reelStop: ['reel-stop.wav', 'reel-stop.mp3'],
        win: ['win-small.mp3', 'win-small.wav'],
        bigWin: ['win-big.mp3', 'win-big.wav'],
        jackpot: ['jackpot.mp3', 'jackpot.wav'],
        error: ['error.wav', 'error.mp3'],
        ambient: ['ambient.mp3', 'ambient.ogg', 'ambient.wav'],
      };
      const fetchAndDecode = async (url) => {
        try{ const res = await fetch(url, { cache:'no-store' }); if(!res.ok) return null; const arr = await res.arrayBuffer(); return await this.ctx.decodeAudioData(arr); }catch{ return null; }
      };
      for(const [key, files] of Object.entries(candidates)){
        if(this.buffers[key]) continue; // don't override if something already loaded
        for(const name of files){
          const buf = await fetchAndDecode(root + name);
          if(buf){ this.buffers[key] = buf; break; }
        }
      }
    },
    ...
    async loadThemeAudio(theme){
      this.init();
      // Remove previously loaded theme-specific buffers but keep global defaults
      if(this.themeKeys && this.themeKeys.size){
        for(const k of this.themeKeys){ delete this.buffers[k]; }
      ...
      for(const [key, url] of entries){
        try{
          ...
          this.buffers[key] = buf; // theme overrides defaults
          this.themeKeys.add(key);
        }catch(e){ /* ignore, fallback to default or synth */ }
      }
    },
...
@font-face{
  ...
  font-display:swap;
    ...
    // Load global default audio first; theme audio overrides; then fallback to synth beeps
    Sound.loadDefaultAudio().catch(()=>{});
  ...
  function startSpin(){
    ...
    const beforeMiddles = state.currentMiddle.slice();
    ...
    for(let i=0; i<state.columns; i++){
      ...
      // Ensure the middle visible element at the end (transform=0) is the target
      if(inner.children[1]) inner.children[1].replaceWith(createSymbolEl(finalMid, state.symbolType));

      pending++; reel.classList.add('spinning'); reelsEl.classList.add('spinning-active');
      const duration = 2.5 + i*0.4; // slowed down a bit
      setTimeout(()=>{
    ...
    // Debug: plan for this spin
    try{ logSpinPlan(beforeMiddles, finalMiddles); }catch{}
  ...
  function onSpinEnd(finalMiddles){
    ...
    try{ logSpinResult(finalMiddles); }catch{}
    ...
    if(state.win>0){
      ...
      setTimeout(()=>{ for(let i=0;i<state.columns;i++){ const reel = reelsEl.children[i]; const inner = reel.querySelector('.reel-inner'); const symbol = inner.children[1]; symbol.classList.add('lose-fade'); } }, 300);
  ...
  function highlightWin(_m, info){
    ...
    for(let c=info.start;c<=info.end;c++){
      const reel = reelsEl.children[c]; const inner = reel.querySelector('.reel-inner');
      // Middle visible row is index 1
      const el = inner.children[1];
  ...
  function attachEvents(){
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if(k===' '){
        ...
        if(state.loseOverlayActive){ hideLoseMessage(); return; }
        try{ Sound.uiClick(); }catch{}
        startSpin();
      }
      else if(map[k]!==undefined){ e.preventDefault(); if(state.holdMode){ try{ Sound.uiClick(); }catch{} } toggleHold(map[k]); }
    });
  }
